...
if (!function_exists('plugin_og_meta_tags')) {
    ...
    function plugin_og_meta_tags() {
        ...
        $is_np = $data_provider->isNp();
        if ($is_np) {
            $og_tags = $data_provider->getPageOgTags();
            if ($og_tags) {
                if ($og_tags['title'] && $og_tags['title'] !== '') {
                    $title = $og_tags['title'];
                }
                if ($og_tags['description'] && $og_tags['description'] !== '') {
                    $description = $og_tags['description'];
                }
                if ($og_tags['url'] && $og_tags['url'] !== '') {
                    $url = $og_tags['url'];
                }
                if ($og_tags['image'] && $og_tags['image'] !== '') {
                    $image = fixImagePaths($og_tags['image']);
                }
            }
        }
        ...
        <?php
        if (is_singular() && has_post_thumbnail($post->ID) && !$is_np) {
