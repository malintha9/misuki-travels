...
function theme_scripts() {
    ...
    wp_localize_script('theme-script', 'screenReaderText', array(
        'expand' => __('expand child menu', 'japantours'),
        'collapse' => __('collapse child menu', 'japantours'),
    ));
}
...
function theme_widgets_init2() {
    register_sidebar(array(
        'name'          => __('Primary Widget Area', 'japantours'),
        'id'            => 'primary',
        'description'   => __('Add widgets here to appear in your sidebar on blog posts and archive pages.', 'japantours'),
...
function _theme_get_excerpt($args = array()) {
	...
	$more_tag                     = isset($args['more_tag'])                     ? $args['more_tag']                     : __('Read more', 'japantours');
...
function theme_get_category_list() {
	return str_replace(
		'<a',
		'<a class="u-textlink"',
		get_the_category_list(_x(', ', 'Used between list items, there is a space after the comma.', 'japantours'))
	);
}

function theme_get_tags_list() {
    return get_the_tag_list('', _x( ', ', 'Used between list items, there is a space after the comma.', 'japantours'));
}
...
function getProductButtonHtml ($button_html, $product, $clickTypeProductbutton = 'add-to-cart', $contentProductbutton = '') {
    ...
    $buttonText = sprintf(__('Add to cart', 'japantours'));
    if ($contentProductbutton !== '') {
        $buttonText =  sprintf(__($contentProductbutton, 'japantours'));
    }
...
function theme_404_content($args = '') {
    $args = wp_parse_args($args, array(
        'error_title' => __('Nothing here', 'japantours'),
        'error_message' => __('It looks like nothing was found at this location. Maybe try a search?', 'japantours')
    ));
    extract($args);
...
function show_all_tours() {
  ...
  $image_url = get_the_post_thumbnail_url($tour_id, 'medium');
    if (!$image_url) {
  ...
  <a href="<?= esc_url(site_url('/index.php/login?redirect_to=' . urlencode('/index.php/book-tour/?tour_id=' . $tour_id))) ?>
...
add_shortcode('tour_list', 'show_all_tours');



//book tour shortcode

function book_tour_form_shortcode() {
    if (!is_user_logged_in()) {
    $redirect = esc_url(add_query_arg('tour_id', intval($_GET['tour_id']), site_url('/index.php/book-tour/')));
    ...
    $price     = get_post_meta($tour_id, 'tour_price', true);
    $duration  = get_post_meta($tour_id, 'tour_duration', true);
    $images    = get_post_meta($tour_id, 'tour_images', true);
    $image_url = (!empty($images)) ? esc_url($images[0]) : 'https://via.placeholder.com/600x300?text=No+Image';

    ob_start();
    ?>
    <div class="book-tour-wrapper" style="max-width: 800px; margin: auto; padding: 20px;">
        <h2>Book Tour: <?= esc_html($title) ?></h2>
        <img src="<?= $image_url ?>" alt="<?= esc_attr($title) ?>" style="width:100%; max-height:300px; object-fit:cover;">
...
add_shortcode('book_tour_form', 'book_tour_form_shortcode');

function custom_login_expiration($expire, $user_id, $remember) {
    return 600; // 600 seconds = 10 minutes
}
add_filter('auth_cookie_expiration', 'custom_login_expiration', 99, 3);
...
function custom_login_shortcode() {
  ...
  $redirect_url = home_url('/book-tour');
    if (isset($_GET['redirect_to']) && !empty($_GET['redirect_to'])) {
        $redirect_url = urldecode(esc_url_raw($_GET['redirect_to']));
    }
            ...
            <div class="card-body">
                ...
                <div class="d-flex justify-content-between mt-3">
                  ...
                  <div class="mt-4 text-center">
                      ...
                      <a  href="<?= esc_url(site_url('/index.php/user-registration-form/')) ?>" class="btn btn-outline-danger mt-2">Register</a>

    <a href="<?= esc_url(site_url('/?loginSocial=google')) ?>" class="btn btn-outline-danger mt-2">
...
function custom_register_shortcode() {
    // if (is_user_logged_in()) return '<p>You are already registered and logged in.</p>';

     if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['custom_register']) && $_POST['custom_register'] === 'yes') {
        // Sanitize inputs
        $username = sanitize_user($_POST['username']);
        $email    = sanitize_email($_POST['email']);
        $phoneNumber    = sanitize_email($_POST['phoneNumber']);
        ...
        if (username_exists($username)) {
            ...
            $error = 'Invalid phone number format. It should be 10 to 15 digits.';
            ...
            $userdata = [
                ...
                'user_email' => $email,
                'role'       => 'subscriber' // Set role if needed
            ];
            ...
            if (!is_wp_error($user_id)) {
                ...
                wp_redirect(site_url('/index.php/book-tour'));
                exit;
            } else {
    ...
    <form method="POST" class="p-4 border rounded shadow-sm bg-white" style="max-width: 400px; margin: 0 auto;">
        <h4 class="mb-3 text-center">Register Your Account</h4>
        <div class="mb-3">
        ...
        <div class="mb-3">
            <input type="number" name="phoneNumber" class="form-control" placeholder="PhoneNumber" required>
        </div>
        <div class="mb-3">
            <input type="password" name="password" class="form-control" placeholder="Password" required>
        </div>
        <input type="hidden" name="custom_register" value="yes">
        <button type="submit" class="btn btn-warning w-100 text-dark">Register</button>
    </form>
<style>
   
    .nsl-container .nsl-clear {
...
function render_tour_booking_form_shortcode() {
//     if (is_user_logged_in()) {
//     $user = wp_get_current_user();
//     return '<p>Welcome back, ' . esc_html($user->display_name) . ' | <a href="' . wp_logout_url(home_url()) . '">Logout</a></p>';
// }
//     if (!is_user_logged_in()) {
//         return '<p>Please <a href="' . wp_login_url() . '">login</a> to book a tour.</p>';
//     }

    $tour_id = isset($_GET['tour_id']) ? intval($_GET['tour_id']) : 0;
    if (!$tour_id) return '<p>Invalid tour selected.</p>';

    $tour_title = get_the_title($tour_id);
    $price = get_post_meta($tour_id, 'tour_price', true);
    $price_display = $price ? '$' . number_format($price, 2) : 'N/A';
    ...
    <div class="card shadow">
        ...
        <div class="card-body">
            <form method="post" action="<?= esc_url(site_url('/process-booking')) ?>">
                ...
                <div class="mb-3">
                    ...
                    <input type="number" name="guests" id="guests" class="form-control" value="1" min="1" required>
                </div>

                <div class="alert alert-info">
                    <strong>Tour:</strong> <?= esc_html($tour_title) ?><br>
                    <strong>Price per Person:</strong> <?= esc_html($price_display) ?>
                </div>
...
add_shortcode('tour_booking_form', 'render_tour_booking_form_shortcode');
...
function blog_sorting() {
    $blog_sorting_options = array(
        'name_asc'  => __('Name, A to Z', 'japantours'),
        'name_desc' => __('Name, Z to A', 'japantours'),
        'date_asc'  => __('Date, old to new', 'japantours'),
        'date_desc' => __('Date, new to old', 'japantours'),
    );
...
function filtered_tours_list_shortcode() {
    $where = sanitize_text_field($_GET['where'] ?? '');
    $when  = sanitize_text_field($_GET['when'] ?? '');
 ...
 <form method="get" class="tour-filter-form" action="<?php echo esc_url( get_permalink() ); ?>">
        <h2>Find Your Tour</h2>
        <div class="form-row">
            <input type="text" name="where" placeholder="Where to?" value="<?php echo esc_attr($where); ?>" />
            <input type="text" id="datepicker" name="when" value="<?php echo esc_attr($when); ?>" placeholder="When" />
...
.form-row button:hover {
   ...
   // if ($query->have_posts()) {
        ...
        echo '</ul>';
    //} else {
     //   echo '<p>No tours found for your search.</p>';
   // }

    wp_reset_postdata();
    return ob_get_clean();
}
add_shortcode('tours_list_filtered', 'filtered_tours_list_shortcode');

    // Load Flatpickr assets

    function load_flatpickr_assets() {
    wp_enqueue_style('flatpickr-style', 'https://cdn.jsdelivr.net/npm/flatpickr/dist/flatpickr.min.css');
    wp_enqueue_script('flatpickr-js', 'https://cdn.jsdelivr.net/npm/flatpickr', [], null, true);
}
add_action('wp_enqueue_scripts', 'load_flatpickr_assets');
...
function custom_send_welcome_email($user_id) {
    ...
    <head>
      <style>
        ...
        .email-wrapper { background: #fff; padding: 20px; border-radius: 8px; box-shadow: 0 0 5px #ccc; }
    ...
    <body>
      <div class='email-wrapper'>
        <div class='header'>Welcome, {$user->user_login}!</div>
        <div class='content'>
          <p>Thank you for registering at <strong>" . get_bloginfo('name') . "</strong>.</p>
          <p>You can now login at <a href='" . wp_login_url() . "'>" . wp_login_url() . "</a>.</p>
            ...
            function resetLogoutTimer() {
                clearTimeout(logoutTimer);
                 logoutTimer = setTimeout(() => {
                    window.location.href = "<?= wp_logout_url(home_url()); ?>";
